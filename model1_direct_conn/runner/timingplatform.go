...
func (b R9NanoPlatformBuilder) Build() *Platform {
	...
	connection := sim.NewDirectConnection("Mesh", b.engine, 1*sim.GHz)
	cpuPorts := []sim.Port{
...
func (b *R9NanoPlatformBuilder) createGPUs(
	...
	for y := 0; y < b.tileHeight; y++ {
		for x := 0; x < b.tileWidth; x++ {
			...
			b.createGPU(x, y, gpuBuilder, gpuDriver, rdmaAddressTable, pmcAddressTable, connection)
		}
	}
}
...
func (b *R9NanoPlatformBuilder) createGPU(
	...
	for _, port := range gpu.Domain.Ports() {
		connection.PlugIn(port, 1024)
	}
