...
func newCacheTracer(timeTeller sim.TimeTeller) *cacheTracer {
	return &cacheTracer{
...
func (t *cacheTracer) StartTask(task tracing.Task) {
	t.Lock()
	defer t.Unlock()
...
func (t *cacheTracer) StepTask(task tracing.Task) {
	// Do nothing
}

// EndTask records the end of the task
func (t *cacheTracer) EndTask(task tracing.Task) {
	t.Lock()
	defer t.Unlock()
