...
func (b *R9NanoGPUBuilder) connectCP() {
	...
	b.internalConn.PlugIn(b.cp.ToRDMA, 1024000)
	b.internalConn.PlugIn(b.cp.ToPMC, 1024)

	b.cp.RDMA = b.rdmaEngine.CtrlPort
	b.internalConn.PlugIn(b.cp.RDMA, 1024000)
...
func (b *R9NanoGPUBuilder) connectL1ToL2() {
	...
	l1ToL2Conn.PlugIn(b.rdmaEngine.ToL1, 1600000)
	l1ToL2Conn.PlugIn(b.rdmaEngine.ToL2, 1600000)
...
func (b *R9NanoGPUBuilder) buildL2TLB() {
	numWays := 64
	entry := b.dramSize / (1 << b.log2PageSize) / uint64(numWays)
	builder := tlb.MakeBuilder().
		WithEngine(b.engine).
		WithFreq(b.freq).
		WithNumWays(numWays).
		WithNumSets(int(entry)).
		WithNumMSHREntry(64).
