...
func (b *R9NanoGPUBuilder) connectCP() {
	...
	b.internalConn.PlugIn(b.cp.ToRDMA, 40)
	b.internalConn.PlugIn(b.cp.ToPMC, 4)

	b.cp.RDMA = b.rdmaEngine.CtrlPort
	b.internalConn.PlugIn(b.cp.RDMA, 1024)
...
func (b *R9NanoGPUBuilder) connectL1ToL2() {
	...
	lowModuleFinder.HighAddress = b.memAddrOffset + 2*mem.GB

	l1ToL2Conn := sim.NewDirectConnection(b.gpuName+".L1toL2",
		b.engine, b.freq)

	b.rdmaEngine.SetLocalModuleFinder(lowModuleFinder)
	l1ToL2Conn.PlugIn(b.rdmaEngine.ToL1, 65536*4)
	l1ToL2Conn.PlugIn(b.rdmaEngine.ToL2, 65536*4)
...
func (b *R9NanoGPUBuilder) createDramControllerBuilder() dram.Builder {
	memBankSize := 2 * mem.GB / uint64(b.numMemoryBank)
	if 2*mem.GB%uint64(b.numMemoryBank) != 0 {
