...
func (b *R9NanoGPUBuilder) connectL1ToL2() {
	...
	lowModuleFinder.HighAddress = b.memAddrOffset + 1*mem.GB
	...
	l1ToL2Conn.PlugIn(b.rdmaEngine.ToL2, 65336)
...
func (b *R9NanoGPUBuilder) createDramControllerBuilder() dram.Builder {
	memBankSize := 1 * mem.GB / uint64(b.numMemoryBank)
	if 1*mem.GB%uint64(b.numMemoryBank) != 0 {
		panic("GPU memory size is not a multiple of the number of memory banks")
	}

	dramCol := 64
	dramRow := 16384
	dramDeviceWidth := 128 / 4
