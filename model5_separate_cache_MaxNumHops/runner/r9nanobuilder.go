...
type R9NanoGPUBuilder struct {
	...
	lowModuleFinderForRDMA  *mem.InterleavedLowModuleFinder
...
func (b *R9NanoGPUBuilder) connectL1ToL2() {
	...
	lowModuleFinderRDMA := mem.NewInterleavedLowModuleFinder(
		1 << b.log2MemoryBankInterleavingSize)
	lowModuleFinderRDMA.ModuleForOtherAddresses = b.rdmaEngine.ToL1
	lowModuleFinderRDMA.UseAddressSpaceLimitation = true
	lowModuleFinderRDMA.LowAddress = b.memAddrOffset
	lowModuleFinderRDMA.HighAddress = b.memAddrOffset + 4*mem.GB

	l1ToL2Conn := sim.NewDirectConnection(b.gpuName+".L1-L2",
		b.engine, b.freq)

	b.rdmaEngine.SetLocalModuleFinder(lowModuleFinderRDMA)
	...
	for _, l2 := range b.l2rCaches {
		lowModuleFinderRDMA.LowModules = append(lowModuleFinderRDMA.LowModules,
			l2.GetPortByName("Top"))
		l1ToL2Conn.PlugIn(l2.GetPortByName("Top"), 64)
	}
...
func (b *R9NanoGPUBuilder) buildL2Caches() {
	...
	for i := 0; i < b.numMemoryBank; i++ {
		...
		cacheName = fmt.Sprintf("%s.L2R_%d", b.gpuName, i)
		l2 = l2Builder.WithInterleaving(
			1<<(b.log2MemoryBankInterleavingSize-b.log2CacheLineSize),
			b.numMemoryBank,
			i,
